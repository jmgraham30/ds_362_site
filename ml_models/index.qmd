...
### Example 1: `tidymodels` Workflow for Regression

The following is an example of the `tidymodels` workflow for regression.  The example uses the `mtcars` dataset and the `parsnip` and `recipes` packages.  The example is adapted from the [Tidy Modeling with R](https://www.tmwr.org/) book [@kuhn2022tidy].
