...
2. We must decide on how we will define and measure error. For regression problems, a typical way to measure error is the **squared-error**. Referring back to @fig-sl-reg, we define the $i$-th **residual** $r_{i}$ to be the vertical (signed) distance between the observed response value $y_{i}$ and the corresponding predicted value $\hat{y}_{i} = \hat{f}({\bf x}_{i})$. That is, 
