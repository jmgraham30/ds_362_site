...
f(X) &= \beta_{0} + \sum_{k=1}^{K}\beta_{k}h_{k}(X) \\
     &= \beta_{0} + \sum_{k=1}^{K}\beta_{k}g(w_{k0} + \sum_{j=1}^{p}w_{kj}X_{j})
\end{align*}
$$
where $g$ is some activation function. Notice that a single layer neural network with $p$ inputs and $K$ hidden units has $1 + K + K + pK$ parameters. 
