...
Convolutional Neural Networks (CNNs) are a type of neural network designed for processing grid-like data, with a primary focus on tasks related to images and other spatial data. CNNs have been instrumental in revolutionizing computer vision and are widely used for tasks like image classification, object detection, and segmentation. @fig-cnn-idea illustrates the general idea of a CNN. That is, rather than learning each pixel of an image, we first learn local features (e.g., edges, shapes) and then combine them to make predictions.


![CNNs work by learning local features an of image through convolution and pooling.](https://www.dropbox.com/scl/fi/ki18hqgaon23oa7cea5eg/10_6.jpeg?rlkey=qplu10zfhezkjiejcw4puju1a&raw=1){#fig-cnn-idea}
...
@fig-cnn provides an illustration of the CNN architecture. For further perspective on CNNs, [view this article](https://towardsdatascience.com/a-comprehensive-guide-to-convolutional-neural-networks-the-eli5-way-3bd2b1164a53).

![Architecture of CNNs.](https://miro.medium.com/v2/resize:fit:720/format:webp/1*uAeANQIOQPqWZnnuH-VEyw.jpeg){#fig-cnn}
...
@fig-rnn provides an illustration of the RNN architecture. For further perspective on CNNs, [view this article](https://medium.datadriveninvestor.com/recurrent-neural-network-with-keras-b5b5f6fe5187).

![Architecture of an RNN.](https://miro.medium.com/v2/resize:fit:640/format:webp/0*xs3Dya3qQBx6IU7C.png){#fig-rnn}
...
1. **keras:**
   ...
   - *Lab from Textbook*: The webpage for the ISLR2 textbook contains a link to a lab that uses the `keras` package. [View lab](https://hastie.su.domains/ISLR2/Labs/Rmarkdown_Notebooks/Ch10-deeplearning-lab-keras.html).
...
3. **torch:**
   ...
   - *Lab from Textbook*: The webpage for the ISLR2 textbook contains a link to a lab that uses the `torch` package. [View lab](https://hastie.su.domains/ISLR2/Labs/Rmarkdown_Notebooks/Ch10-deeplearning-lab-torch.html).
