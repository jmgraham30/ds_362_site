...
The following line of code is not run but demonstrate how to load a data file such as a [comma-separated values file](https://en.wikipedia.org/wiki/Comma-separated_values) (CSV) 
...
The previous code uses the `read_csv` function from the `readr` package (part of `tidyverse`) to directly read in the `egg-production.csv` from the specified location.  
...
Our next step in the course will be to study some common supervised learning algorithms. There are also situations in which a data set does not consist of a natural pairing between predictors and a response. Unpaired or unlabeled data can by analysed using **unsupervised learning** methods. We will address unsupervised learning problems and algorithms later in the course.   
